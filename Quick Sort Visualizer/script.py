...
def quickSort(array, left, right, drawRect):
    if left<right:
        ...
        drawRect(array, [BLUE if x >= left and x < pivot else YELLOW if x == pivot
                        else PURPLE if x > pivot and x <=right else RED for x in range(len(array))])
...
#---adding widgets---
#top label
greeting = Label(top, text = "Quick Sort Visualizer", width = 62, font = ("Courier New", 20, "bold"), background = GREEN_2)
